...
- **硬模式还没做进程级**：`SubscribeKey(block=true)` 是驱动层全局拦截，跟窗口无关。
  要做进程级得「按前台窗口动态订阅/退订」或「拦截后回注」，有键卡住风险，需实机测。见文末 TODO。
...
- **硬模式的进程级占用**：普通模式已用 `HotIf` 实现（只在目标窗口吞键，别处透传）。
  硬模式 `SubscribeKey(block=true)` 是驱动层全局拦截，还没做进程级。推荐方案：定时器盯前台
  窗口，目标窗口激活才 `SubscribeKey`、失焦就 `UnsubscribeKey`，别处自然透传。**风险**：漏掉
  失焦事件或异常退出时正处于已订阅态，键会卡死——必须有兜底（`OnExit`/定时器里强制 Unsubscribe，
  或超时自动退订）。需实机测，本机自动化环境测不了驱动级透传。
