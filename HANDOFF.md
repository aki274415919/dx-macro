...
- **进程级占用（普通模式）**：SendInput 分支的用户热键用 `HotIf(HotkeyContextActive.Bind(name))`
  注册，只在「当前前台窗口有匹配配置」时激活并吞键；其它进程上这个键**原样透传**。
  `active_window=""` 的全局热键始终有兜底配置，所以处处生效。注册后要 `HotIf()` 复位，
  否则上下文会带给后面注册的暂停/退出/辅助热键。
- **进程级占用（硬模式）**：不再一注册就全局拦截。`RegisterHardHotkey` 只 `ReserveHotkey`
  占位查扫描码冲突；真正的拦截由 `ReconcileHardHotkeys` 按前台窗口**动态订阅/退订**：
  目标窗口激活 → `EnsureSubscribed`（`SubscribeKey block=true`）；不在 → `EnsureUnsubscribed`
  （`UnsubscribeKey`，键透传）；暂停时全退订。定时器每 `poll_ms`(默认200) 调一次，
  `TogglePause` 立刻调一次。**对账式、自愈**：漏掉一次窗口切换下一拍纠正；进程崩溃/退出时
  AHI 上下文销毁自动放开所有键；`OnExitHandler` 再 `UnsubscribeAll` 兜底。
  → 真驱动实测过：EnsureSubscribed/Unsubscribed/UnsubscribeAll 幂等、不崩不卡。
- `#PollMs 200`（也叫 `#PollInterval`）配置对账间隔，下限在 Main 里 clamp 到 30ms。
  **只是软件定时器轮询 `WinActive`，不产生硬件中断**；订阅/退订只在窗口切换那一下发生，平时不动。
- `HasMethod(Backend, "EnsureSubscribed")` 判断是不是硬后端（比按类名判断更稳，也好测）。
- Interception 后端用 `SubscribeKey` 接管无修饰用户热键；组合热键和工具控制键仍用 AHK `Hotkey()`。
...
- `Ctrl+Alt+H` 打开帮助窗口，一屏列出所有功能、快捷键和脚本写法。
- 托盘右键菜单 = 全部功能：识别按键 / 查进程 / 录制 / 编辑 / 重载 / 暂停 / 帮助 / 退出，
  每项都标了快捷键。启动的 TrayTip 只提示「右键托盘看全部」，不再往小气泡里塞快捷键（记不住）。
- 快捷键显示用 `HumanHotkey()` 把 `^!x` 转成 `Ctrl+Alt+X`；暂停/退出键可在脚本配置，标签会跟着变。
- `#DxHardInput on` 但驱动没装：弹一个能点去下载页的框，然后回退到 SendInput 继续跑。

## 支持的 `.dxm` 语法

- 指令：`#Requires`、`#AskAdmin`、`#RequireAdmin`、`#DxHardInput`、`#PauseKey`、`#ExitKey`、`#HotIf`、`#Repeat`、`#InterceptionVid`、`#InterceptionPid`、`#InterceptionInstance`、`#PollMs`、`#Include`、`#Block`
- 热键：`Name::`
...
- 硬模式进程级占用**已完成**（定时器对账 + `poll_ms` 可调，见上）。真驱动实测了订阅/退订机制；
  仍建议在真实游戏里验一次「切出游戏后小键盘键在别的程序里恢复正常」。
- 硬模式方向键/扩展键的实际发送（`SendKeyEvent` 认不认 0x100 扩展位）仍需真实目标程序确认。
