...
## ⏸ 会话进度（2026-07-09，重启后从这里继续）

**状态：Interception 驱动已安装，等一次重启才生效。开机后按下面走。**

已完成：
- 下载官方 Interception v1.0.1（`.downloads\`，SHA256 `AD03…AF10`，与仓库 vendored 的 dll 逐字节一致）。
- 以管理员跑了 `install-interception.exe /install`，退出码 0。
- 已验证：`keyboard.sys`/`mouse.sys` 进了 `System32\drivers`；注册表服务 `keyboard`/`mouse` 已建；
  键盘类 `UpperFilters = keyboard, kbdclass`，鼠标类 `UpperFilters = mouse, mouclass`。
- 用户选择关机（不是重启）。

**开机后要做（司机上游过滤驱动只在设备重新枚举时加载）：**
1. 验证驱动真的加载了：`interception_create_context()` 应返回非 0。
   跑 `InterceptionDriverPresent()`（在 `lib\Backends.ahk`）或直接 DllCall 探测。
   **若仍返回 0**：Windows 快速启动可能让「关机→开机」跳过驱动重载，让用户做一次完整**重启**。
2. 跑项目根的 `Monitor.ahk`，让用户在目标键盘上按键，记下 VID/PID。
3. 把 VID/PID 填进 `dx-macro.dxm`，`#DxHardInput on`（现在已经是 on）。
4. 实机跑通 Interception 后端：确认 `Send "{Left}"` 走驱动发得出去。
   注意小键盘 NumLock 扫描码陷阱（见下文 Interception 一节）。

卸载驱动：管理员跑 `.downloads\...\command line installer\install-interception.exe /uninstall` 再重启。
...
| `selftest.ahk` | 自检 |
| `run.bat` | 开发时跑脚本版 |
| `Monitor.ahk`, `lib/AutoHotInterception.*`, `lib/CLR.ahk`, `lib/x64`, `lib/x86` | AutoHotInterception，第三方，Git 忽略 |
...
正确布局：`Monitor.ahk` 在根，其余 AHI 文件在 `lib\`（含 `lib\x64\`、`lib\x86\`）。
...
49 条断言，退出码 0 = 全过。自检不发送真实键盘输入，用 `MockBackend` 检查解析、校验、
动作展开、松键兜底、硬输入指令、管理员指令、`#HotIf` 解析、`ParseSendGroups` 拆组、
`RunSend` 路由到后端、`EmitRecording` 录制转换。

最后一条断言是回归护栏：驱动没装时 `InterceptionBackend()` 必须抛 `DriverMissingError`。
...
2. AHI 文件按上面的布局放好。
3. 跑根目录的 `Monitor.ahk`，在键盘上按个键，记下 VID/PID。
4. 脚本里写：
...
1. `IsSet(AutoHotInterception)` —— AHI 有没有 include
2. `InterceptionDllPath()` / `AutoHotInterception.dll` —— 文件在不在
...
- `#SingleInstance Force`：同一时刻只能跑一个 `.dxm`。编辑器里点「保存并运行」会
  拉起新实例并顶掉当前进程，所以那行代码后面不能再有 `MsgBox`。
